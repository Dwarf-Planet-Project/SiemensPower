...
model splitterMixer_testTubeOwnMedia_simpleInput "Test case for SplitterMixer"
...
SiemensPower.Components.Junctions.SplitterMixer splitter(
  ...
  extent={{-10,-10},{10,10}},
  rotation=90)));
  Blocks.Smoothing smoothing annotation (Placement(transformation(
  extent={{-10,-10},{10,10}},
  rotation=180,
  origin={54,-8})));
  Boundaries.WaterSourceMH watersource_mh2( h_start=
                                         2000e3, m_flow_start=
                                                         10,
    use_h_in=false,
    use_m_flow_in=true)
    annotation (Placement(transformation(extent={{48,-46},{30,-28}},   rotation=
     0)));
...
equation
  ...
  connect(smoothing.y, watersource_mh2.m_flow_in) annotation (Line(
      points={{49,-8},{42.6,-8},{42.6,-31.6}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(watersource_mh2.port, pipe3.portIn) annotation (Line(
      points={{30,-37},{22,-37},{22,-2},{8,-2}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(step.y, smoothing.u) annotation (Line(
      points={{75,22},{70,22},{70,-8},{63,-8}},
      color={0,0,127},
      smooth=Smooth.None));
...
end splitterMixer_testTubeOwnMedia_simpleInput;
